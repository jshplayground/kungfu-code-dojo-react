...
>(({ style, className, transition, ...props }, ref) => {
  ...
  return (
    <motion.img
      ref={ref}
      className={cn(
        "relative z-10 size-auto max-h-full max-w-full rounded-xl object-cover shadow-lg",
        className
      )}
      style={{ scale, ...style }}
      {...props}
...
>(({ className, transition, ...props }, ref) => {
  return (
    <motion.button
      ...
      ref={ref}
      className={cn(
        "group relative flex w-fit items-center rounded-full border border-kungfu-red bg-dark-bg/10 px-4 py-2 shadow-[0px_4px_24px_#E50914] transition-colors hover:bg-dark-bg/50",
        className
      )}
      {...props}
    />
