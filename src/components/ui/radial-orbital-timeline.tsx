...
export default function RadialOrbitalTimeline({
  ...
  return <div className="w-full h-[110vh] flex flex-col items-center justify-center bg-transparent overflow-hidden" ref={containerRef} onClick={handleContainerClick}>
      <div className="relative w-full max-w-5xl h-full flex items-center justify-center">
        ...
        transform: `translate(${centerOffset.x}px, ${centerOffset.y}px)`
          ...
          <div className="absolute w-24 h-24 rounded-full bg-gradient-to-br from-kungfu-red via-tech-neon-green to-kungfu-red animate-pulse flex items-center justify-center z-10">
            ...
            <div className="absolute w-32 h-32 rounded-full border border-white/10 animate-ping opacity-50" style={{
            animationDelay: "0.5s"
          }}></div>
          ...
          return <TimelineNode key={item.id} item={item} position={position} isExpanded={!!isExpanded} isRelated={isRelated} isPulsing={!!isPulsing} toggleItem={toggleItem} timelineData={timelineData} ref={(el: HTMLDivElement | null) => nodeRefs.current[item.id] = el} />;
        })}
        </div>
      </div>
    </div>;
